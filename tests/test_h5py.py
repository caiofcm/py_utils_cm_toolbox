...
def test_dic_plain(dic_plain):
    HDF5_FILE = h5py.File('aa.h5', 'w')
    d = dic_plain
    h5py_simple.save_dict_to_file(HDF5_FILE, d)
    HDF5_FILE.close()
...
def test_key_with_zeros():
    new = h5py_simple.key_with_zeros(0, 10)
    assert new == '0'
    new = h5py_simple.key_with_zeros(9, 10)
    assert new == '9'
    new = h5py_simple.key_with_zeros(10, 11)
    assert new == '10'
    new = h5py_simple.key_with_zeros(1, 11)
    assert new == '01'
    new = h5py_simple.key_with_zeros(2, 100)
    assert new == '02'
    new = h5py_simple.key_with_zeros(2, 101)
    assert new == '002'
    new = h5py_simple.key_with_zeros(59, 101)
    assert new == '059'
